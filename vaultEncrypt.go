...
//export VaultEncrypt
func VaultEncrypt(text *C.char, vaultAddr *C.char, vaultToken *C.char, enginePath *C.char, keyName *C.char, tlsSkipVerify C.int) *C.char {
	...
	if err != nil {
		C.ereport_error(C.CString(fmt.Sprintf("Cannot encrypt data (%v): %s", plainText, err)))
	}
	...
	C.ereport_debug(C.CString(fmt.Sprintf("cipher text: %s", base64Cipher.(string))))
	return C.CString(base64Cipher.(string))

}

//export VaultDecrypt
func VaultDecrypt(cipher *C.char, vaultAddr *C.char, vaultToken *C.char, enginePath *C.char, keyName *C.char, tlsSkipVerify C.int) *C.char {
	...
	if err != nil {
		C.ereport_error(C.CString(fmt.Sprintf("Cannot decrypt data (%v): %s", base64Cipher, err)))
	}
	...
	return C.CString(fmt.Sprintf("%s", decryptText))
}
